...
/**
 * 친구정보 파일 저장 및 관리 구현체
 * RandomAccessFile 활용(바이트스트림)
 * @author 김기정
 */
public class FileFriendRepository implements FriendRepository {

	/** 파일 저장 경로 */
	// 경로와 파일은 바뀔 일이 없고 외부 공개 이유 없으니 프라이빗 상수
	...
	private static final int NAME_LENGTH = 10; // 이름(10바이트) 이름 석자가 보통 10바이트 정도 필요함(약간 넉넉).
	private static final int AGE_LENGTH = 4; // 나이(4바이트)
	private static final int WEIGHT_LENGTH = 8; // 몸무게(8바이트)
	private static final int TELEPHONE_LENGTH = 40; // 전화번호(40바이트) 총 20자 저장 가능한 공간.
	...
	/** 저장된 친구(레코드)수 */
	// 레코드? 친구 정보가 저장된 하나의 단위. 이름, 나이, 몸무게, 전화번호
	private int recordCount = 0;

	public FileFriendRepository() throws IOException {
		// 읽고 쓰기 가능한 RandomAccessFile 객체 생성
		file = new RandomAccessFile(FILE_PATH, "rw"); // 아무것도 기록되지 않은 상태에서 생성만 됨. 스트림이 생성되면 자동으로 빈 파일 생성.
		// 저장된 파일이 있는 경우..
		if (file.length() != 0) {
	...
	public void save(Friend friend) throws IOException {
		...
		// 새로운 레코드(친구) 추가
		// 10 + 4 + 8 + 40
		String name = friend.getName(); // file.writeUTF(name); 이거 왜 못쓰나? 친구 이름은 10바이트로 저장하기로 했기에,걍 wireUTF고 하면 몇바이트인지 알 수가 없음. 62바이트 구조가 무너진다.
		...
		// 10바이트(5글자)로 이름 저장 
		for (int i = 0; i < (NAME_LENGTH / 2); i++) { //글자수까지 반복하는데, 한 글자는 2바이트니까 2로 나눈다.
			// EX) "김기정 "
			file.writeChar((i < charCount ? name.charAt(i) : ' ')); //이름은 무조건 공백까지 합해서 10바이트로 채운다.
			//3항 연산자. i 가 charCount보다 작으면 name.charAt(i)이고 아니면 ' ' .
		}

		// 나이(4바이트) 이름은 인트니까 무조건 4바이트. 참 쉽죠?
		file.writeInt(age);
		// 몸무게(8바이트)
		...
		// 등록된 레코드(친구) 수 1 증가
		file.seek(0);
		file.writeInt(++recordCount); // 먼저 증가시키고 증가된 값을 파일에 기록한다. 
	}
	...
	private Friend read(int index) throws IOException {
		...
		for (int i = 0; i < (NAME_LENGTH / 2); i++) {
			name += file.readChar(); //"김재훈  " 한글자 한글자 붙여서 name 변수에 넣어줌.
		}
		name = name.trim(); //끝에 공백 제거.
		age = file.readInt();
